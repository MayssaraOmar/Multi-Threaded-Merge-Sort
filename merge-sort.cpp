...
public:
    ...
    void execute(){
        arguments* args = new arguments(*list, 0, (*list).size()-1);
        mergeSort((void *) args);
        delete(args);
        return;
    }
private:
    ...
    static void* mergeSort(void* args){
        ...
        int mid = (start+end)/2;
        int rc;
        arguments* args1 = new arguments(*curr_args->list, start, mid);
        rc = pthread_create(&threads[0], NULL, mergeSort, (void *) args1);
        if (rc){
            cout << "Error:unable to create thread," << rc << endl;
            exit(-1);
        }
        arguments* args2 = new arguments(*curr_args->list, mid+1, end);
